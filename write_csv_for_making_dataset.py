...
def generate_set(data, process_name, filename):
    ...
    for idx, file in enumerate(data):
        ...
        try:
            ...
            bird_id    = os.path.basename(file).split('.')[0]
            bird_label = os.path.basename(os.path.dirname(file))
            df = df.append({'id': bird_id, 'name': bird_label}, ignore_index = True)
        '''
        extension = file.split('.')[-1]
        bird_id = os.path.basename(file).split('.')[0]
        bird_label = os.path.basename(os.path.dirname(file))
        write_csv('multitasking.csv', [bird_id, bird_label, extension])
